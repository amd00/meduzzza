					...
					m_updater(NULL), m_full_scan_in_progress(false), m_statist(new Statist())
{
	...
	m_engine = new ClamavEngine(m_settings.threadCount(), m_db_dir.absolutePath());
...
bool Manager::init()
{
	...
	connect(m_engine, SIGNAL(procScanStartedSignal(const QString&, qint32)), this, SIGNAL(procScanStartedSignal(const QString&, qint32)));
	connect(m_engine, SIGNAL(fileScanCompletedSignal(const QString&)), this, SIGNAL(fileScanCompletedSignal(const QString&)));
	connect(m_engine, SIGNAL(procScanCompletedSignal(const QString&, qint32)), this, SIGNAL(procScanCompletedSignal(const QString&, qint32)));
	...
	connect(m_engine, SIGNAL(procVirusDetectedSignal(const QString&, qint32, const QString&)), this, SIGNAL(procVirusDetectedSignal(const QString&, qint32, const QString&)));
	connect(m_engine, SIGNAL(errorSignal(const QString&, const QString&)), this, SIGNAL(errorSignal(const QString&, const QString&)));
	
	connect(m_engine, SIGNAL(dirScanStartedSignal()), this, SIGNAL(dirScanStartedSignal()));
	connect(m_engine, SIGNAL(memScanStartedSignal()), this, SIGNAL(memScanStartedSignal()));
	connect(m_engine, SIGNAL(dirScanCompletedSignal()), this, SIGNAL(dirScanCompletedSignal()));
	connect(m_engine, SIGNAL(memScanCompletedSignal()), this, SIGNAL(memScanCompletedSignal()));
	connect(m_engine, SIGNAL(scanStoppedSignal()), this, SIGNAL(scanStoppedSignal()));
	
	connect(m_engine, SIGNAL(dirScanCompletedSignal()), this, SLOT(dirScanCompletedSlot()));
...
{
	QEventLoop loop;
	loop.connect(m_engine, SIGNAL(dirScanCompletedSignal()), SLOT(quit()));
	loop.connect(m_engine, SIGNAL(scanStoppedSignal()), SLOT(quit()));
